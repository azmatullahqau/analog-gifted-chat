...
import { FlatList, ImageBackground, Keyboard, Platform, StyleSheet, Text, TouchableOpacity, View, } from 'react-native'
import LoadEarlier from './LoadEarlier'
import Message from './Message'
import Color from './Color'
import { StylePropType, warning } from './utils'
import TypingIndicator from './TypingIndicator'
import { debounce } from '../../../app/utils/CustomUtils'
...
export default class MessageContainer extends React.PureComponent {
    constructor () {
        ...
        this.renderRow = ({ item, index }) => {
            ...
            if (messages && user) {
                ...
                const messageProps = {
                    ...
                    inverted,
                    nextMessage,
                    isInViewPort: this.state.inViewPortMessages?.includes(String(item._id)),
        ...
        this.onViewableItemsChanged = (messages) => {
            this.setState({
                inViewPortMessages: messages?.viewableItems?.map((message) => message?.key) || []
            })
        }
        this.viewabilityConfigCallbackPairs = [{
            ...
            onViewableItemsChanged: debounce(this.onViewableItemsChanged, 2000)
        }]
    }
    componentDidMount () {
    ...
    render () {
        const { inverted, chatBackgroundImage, chatBackgroundColor } = this.props
        return (<View style={[this.props.alignTop ? styles.containerAlignTop : styles.container, {backgroundColor: chatBackgroundColor}]}>
            <ImageBackground
